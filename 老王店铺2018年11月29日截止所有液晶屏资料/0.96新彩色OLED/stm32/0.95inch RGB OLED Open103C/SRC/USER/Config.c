...
static void port_init(void) 
{
...
#ifdef INTERFACE_SPI
	//SPI
	tGPIO.GPIO_Pin = SSD1331_SCLK_PIN;	 	 		 
	tGPIO.GPIO_Speed = GPIO_Speed_50MHz;
	tGPIO.GPIO_Mode = GPIO_Mode_AF_PP; 	
	GPIO_Init(SSD1331_SCLK_GPIO, &tGPIO);
	
	//SPI
	tGPIO.GPIO_Pin = SSD1331_SDIN_PIN;	 	 		 
	tGPIO.GPIO_Speed = GPIO_Speed_50MHz;
	tGPIO.GPIO_Mode = GPIO_Mode_AF_PP; 	
	GPIO_Init(SSD1331_SDIN_GPIO, &tGPIO);
#endif
	...
	/*----------------------------------------------------------------------------------*/
	//SSD1331 
	tGPIO.GPIO_Pin = SSD1331_RES_PIN;     // RES				 
	tGPIO.GPIO_Speed = GPIO_Speed_50MHz;
	tGPIO.GPIO_Mode = GPIO_Mode_Out_PP;	
	GPIO_Init(SSD1331_RES_GPIO, &tGPIO);
	
	tGPIO.GPIO_Pin = SSD1331_DC_PIN;     // DC				 
	tGPIO.GPIO_Speed = GPIO_Speed_50MHz;
	tGPIO.GPIO_Mode = GPIO_Mode_Out_PP;	
	GPIO_Init(SSD1331_DC_GPIO, &tGPIO);
	
	tGPIO.GPIO_Pin = SSD1331_CS_PIN;     // CS				 
	tGPIO.GPIO_Speed = GPIO_Speed_50MHz;
	tGPIO.GPIO_Mode = GPIO_Mode_Out_PP;	
	GPIO_Init(SSD1331_CS_GPIO, &tGPIO);
	/*----------------------------------------------------------------------------------*/
}
