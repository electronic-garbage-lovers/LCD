...
{ 	
  ...
  Delay_mS(50);
	...
	SPI_SendByte(SPI_DATA,0x0F);         //Vo=12.V  0x0F;0x03
	...
	SPI_SendByte(SPI_DATA,0x05);         //Bias=1/12
	...
	SPI_SendByte(SPI_DATA,0xA1);         //这个地方必须设置为162，否则没有显示
	SPI_SendByte(SPI_DATA,0x20);         //Nline=off
	
	SPI_SendByte(SPI_COMMAND,0xBC);      //Data scan direction（数据扫描方向)
	SPI_SendByte(SPI_DATA,0x00);         //CL=0;MX=0;MY=0

	SPI_SendByte(SPI_COMMAND,0x0C);      //D0=1
